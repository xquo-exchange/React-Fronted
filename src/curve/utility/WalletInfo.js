...
export async function getWalletDetails(pool, externalProvider) {
  try {
    ...
    const tokenBalancesInPool = (await pool.stats.underlyingBalances()).map(b => num(b));
    ...
    if (pool.underlyingCoins && Array.isArray(pool.underlyingCoins)) {
      ...
      tokenBalances = pool.underlyingCoins.map((symbol, index) => {
        const tokenAmountInPool = tokenBalancesInPool[index];
